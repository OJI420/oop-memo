...
他の言語では、フィールドの値をセットするには __setter__ (値をセットするためだけのメソッド)、ゲットするには __getter__ (フィールドの値をゲットするだけのメソッド)を使うが、pythonでは推奨されていない。

なお、__PySide__ 等の 他言語をラップしたライブラリでは、__setter__ や __getter__ を使用しなければならない。
